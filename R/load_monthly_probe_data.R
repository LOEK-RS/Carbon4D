...
load_monthly_probe_data <- function(path){
  ...
  csv_list_monthly_probe_data = lapply(csv_files_monthly_probe_data,function(i){read.csv(i, header=F,skip = 1)})
  ...
  csv_list_monthly_probe_data <- lapply(csv_list_monthly_probe_data, setNames, nm = colnames)
